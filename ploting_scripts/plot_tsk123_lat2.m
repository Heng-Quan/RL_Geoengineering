...
dTS_2CO2 = mean(TS_2CO2 - TS_original,1)';
dTS_ref_tsk1 = TS_ref_tsk1(end,:)' - mean(TS_original,1)';
dTS_ref_tsk2 = TS_ref_tsk2(end,:)' - mean(TS_original,1)';
dTS_ref_tsk3 = TS_ref_tsk3(end,:)' - mean(TS_original,1)';
dTS_RL_tsk1 = TS_RL_tsk1(end,:)' - mean(TS_original,1)';
dTS_RL_tsk2 = TS_RL_tsk2(end,:)' - mean(TS_original,1)';
dTS_RL_tsk3 = TS_RL_tsk3(end,:)' - mean(TS_original,1)';
...
set(gca,'FontSize',18)
title('(a) Stabilize CC',fontsize=20)
...
title('(c) Revert tropical CC',fontsize=20)
...
hold on
plot(sin(lat*pi/180),dTS_RL_tsk1,'-k','LineWidth',2)
hold on
...
text(-0.5,0.5,'$r[\Delta T]: -0.31\mathrm{K} \rightarrow -0.28\mathrm{K}$',...
...
hold on
plot(sin(lat*pi/180),dTS_RL_tsk2,'-k','LineWidth',2)
hold on
plot(sin(lat*pi/180),zeros(32,1),'-k','LineWidth',0.5)
hold on
patch([sin(lat*pi/180)' fliplr(sin(lat*pi/180)')], [-TS_ctl_std fliplr(TS_ctl_std)],...
    'k','FaceAlpha',0.1)
text(-0.5,0.5,'$r[\Delta T]: -0.47\mathrm{K} \rightarrow -0.39\mathrm{K}$',...
    'Interpreter','latex','FontSize',18)
...
hold on
plot(sin(lat(11:22)*pi/180),dTS_RL_tsk3(11:22),'-k','LineWidth',2)
hold on
...
lh.Color(4) = 0.3;
hold on
lh = plot(sin(lat(1:11)*pi/180),dTS_RL_tsk3(1:11),'-k','LineWidth',2);
lh.Color(4) = 0.3;
hold on
...
lh.Color(4) = 0.3;
hold on
lh = plot(sin(lat(22:32)*pi/180),dTS_RL_tsk3(22:32),'-k','LineWidth',2);
lh.Color(4) = 0.3;
hold on
...
patch([sin(lat*pi/180)' fliplr(sin(lat*pi/180)')], [-TS_ctl_std fliplr(TS_ctl_std)],...
    'k','FaceAlpha',0.1)
text(-0.5,0.5,'$r[\Delta T]: -0.52\mathrm{K} \rightarrow -0.16\mathrm{K}$',...
    'Interpreter','latex','FontSize',18)
%ylabel('$K$','Interpreter','latex')
ylim([-1,1])
set(gca,'XTick',[-1,-0.71,-0.5,0,0.5,0.71,1])
...
set(gca,'FontSize',18)
title('(f)',fontsize=20)
legend('constant SG','RL',fontsize=20)
...
patch([sin(lat*pi/180)' fliplr(sin(lat*pi/180)')], [-PminusE_ctl_std fliplr(PminusE_ctl_std)],...
    'k','FaceAlpha',0.1)
text(-0.75,0.08,'$r[\Delta P - \Delta E]: -0.020\mathrm{m/yr} \rightarrow -0.016\mathrm{m/yr}$',...
    'Interpreter','latex','FontSize',18)
ylabel('$\Delta \overline{P} - \Delta \overline{E}$ (mm/yr)',...
    'Interpreter','latex')
%legend('2xCO2','constant SG','RL trained')
set(gca,'Xtick',[-1,-0.71,-0.5,0,0.5,0.71,1])
xticklabels([-90,-45,-30,0,30,45,90])
...
patch([sin(lat*pi/180)' fliplr(sin(lat*pi/180)')], [-PminusE_ctl_std fliplr(PminusE_ctl_std)],...
    'k','FaceAlpha',0.1)
text(-0.75,0.08,'$r[\Delta P - \Delta E]: -0.024\mathrm{m/yr} \rightarrow -0.016\mathrm{m/yr}$',...
    'Interpreter','latex','FontSize',18)
%ylabel('$mm/yr$','Interpreter','latex')
%legend('2xCO2','constant SG','RL trained')
set(gca,'Xtick',[-1,-0.71,-0.5,0,0.5,0.71,1])
xticklabels([-90,-45,-30,0,30,45,90])
set(gca,'Ytick',[-0.05,0,0.05])
