classdef CAM_env_smooth2 < rl.env.MATLABEnvironment    
    ...
    properties
        ...
        BNDTVAER_file = ['/gpfs/share/home/1800011374/',...
            'CAM3_SOM/AerosolMass_V_64x128_varying2.nc'];
        init_file = '';
        init_file_clm = '';
        init_head = ['/gpfs/share/home/1800011374/',...
            'SRM_RL_CAM2/SRM_RL_run.cam2.i.0'];
        init_head_clm = ['/gpfs/share/home/1800011374/',...
            'SRM_RL_CAM2/SRM_RL_run.clm2.i.0']
        output_head = ['/gpfs/share/home/1800011374/',...
            'SRM_RL_CAM2/SRM_RL_run.cam2.h0.0'];
    ...
    methods              
        ...
        function [Observation,Reward,IsDone,LoggedSignals] = ...
            ...
            year = 100 + fix((this.step_num-1)/12);
            ...
            if this.step_num > 1
                this.init_file = [this.init_head, num2str(year),...
                    '-', num2str(month,'%02d'), '-01-00000.nc'];
                this.init_file_clm = [this.init_head_clm, num2str(year),...
                    '-', num2str(month,'%02d'), '-01-00000.nc'];
            end
            ...
            ['iyear_ad=20' num2str(year-50) ', '],...
            'scenario_ghg = ''ramp_co2_only'', ',...
            'ramp_co2_annual_rate = 1.0, ',...
            'ramp_co2_cap = 2.0, ',...
            'ramp_co2_start_ymd = 00500101, ',...
            'co2vmr=0.0003900, ',...
            'nrefrq=0, ',...
            ['ncdata=''' this.init_file ''', '],...
            'bndtvaer=''/gpfs/share/home/1800011374/CAM3_SOM/AerosolMass_V_64x128_varying2.nc'', ',...
            'bndtvs=''/gpfs/share/home/1800011374/CAM3_SOM/sst_HadOIBl_bc_64x128_clim_SOM_c021214.nc'', ',...
            'inithist=''MONTHLY''/ ',...
            ['&clmexp finidat=''' this.init_file_clm '''/" '],...
            '-o /gpfs/share/home/1800011374/CAM3_SOM/namelist_SRM_RL2'];
            
            system(namelist_command);
            
            % run CAM for 1 month
            while exist('/gpfs/share/home/1800011374/CAM3_SOM/namelist_SRM_RL2','file') == 0
                pause(1)
            ...
            '/gpfs/share/home/1800011374/CAM3_SOM/namelist_SRM_RL2'];
            ...
            delete('/gpfs/share/home/1800011374/CAM3_SOM/namelist_SRM_RL2');
            output_file_new = [this.output_head, num2str(year,'%02d'),...
                '-', num2str(month,'%02d'), '_copy', '.nc'];
            movefile(output_file,output_file_new);
            !> '/gpfs/share/home/1800011374/SRM_RL_CAM2/SRM_RL_run.out'
        ...
        function InitialObservation = reset(this)
            ...
            this.step_num = 0;
            this.init_file = ['/gpfs/share/home/1800011374/CAM3_SOM2/',...
                'output_dir/2xCO2_run.cam2.i.0100-01-01-00000.nc'];
            this.init_file_clm=['/gpfs/share/home/1800011374/CAM3_SOM2/',...
                'output_dir/2xCO2_run.clm2.i.0100-01-01-00000.nc'];
            
            % TS_2xCO2_control ?
            load('SRM_CAM_ref.mat','TS_2xCO2');
            this.TS_buffer = TS_2xCO2;
            this.TS_smooth = mean(mean(TS_2xCO2,3,'omitnan'),1);
    ...
    methods               
        ...
        function Initialize_properties(this)
            ...
            this.TS_initial = mean(mean(TS_2xCO2,3,'omitnan'),1);
            this.TS_smooth = mean(mean(TS_2xCO2,3,'omitnan'),1);
            this.TS_2CO2 = mean(mean(TS_2xCO2,3,'omitnan'),1);
            this.TS_buffer = TS_2xCO2;
    ...
    methods (Access = protected)
        ...
        function envUpdatedCallback(this)
            ...
            saveas(gcf,'/gpfs/share/home/1800011374/SRM_RL_CAM2/temp.jpg')
        end
    end
end
